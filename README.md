...
- The `api-docs.yaml` file is automatically generated by Fastify's OpenAPI plugin. You can access the latest version at `http://localhost:8080/docs/yaml` when running locally. Use `./bin/apidocgen.sh` to regenerate this file after making schema changes in the code.
...
- **ContentVersion Cleanup Tool**: The `bin/cleancontent.sh` script helps manage storage space by cleaning up ContentVersion records created during testing. Use `./bin/cleancontent.sh <org-alias>` to find and delete Anthem-related ContentVersion records. This is useful when hitting storage limits in scratch orgs during development.
- **ApexStubs Tool**: The `bin/apexstubs.sh` script extracts dynamically generated Apex classes from Salesforce External Services and HerokuAppLink integrations. This tool uses Playwright to automatically discover and download the Apex stub classes that Salesforce generates for your Heroku service integrations. Use `./bin/apexstubs.sh` to extract the current Apex classes, which is useful for understanding the generated API structure and debugging integration issues.

### Extract Generated Apex Classes
To see the dynamically generated Apex classes that Salesforce creates for your HerokuAppLink integration:
```bash
./bin/apexstubs.sh
```

This tool will:
- Automatically discover all dynamic Apex classes in your org
- Extract the generated Apex code for each class
- Save the files to `bin/apexstubs/downloads/`
- Show you the exact API structure that Salesforce expects

This is particularly useful for:
- Understanding the generated API structure
- Debugging integration issues
- Verifying that your OpenAPI schema changes are reflected in the generated classes
